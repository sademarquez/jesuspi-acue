document.addEventListener('DOMContentLoaded', function () {
    ...
    // --- Carrusel (Pensamiento y Acción) --- NEW ELEMENTS
    ...
    function initializeCarousel() {
        ...
        carouselContentData.forEach((item, index) => {
            ...
            slide.className = 'carousel-slide flex-none w-full h-full relative'; // flex-none ensures it doesn't shrink
            slide.innerHTML = `
        ...
        // Set initial slide width (important for calculation)
        const slideWidth = carouselTrack.firstElementChild ? carouselTrack.firstElementChild.clientWidth : 0;
        if (slideWidth > 0) {
    ...
    function moveToCarouselSlide(index) {
        ...
        let newIndex = index;
        if (newIndex < 0) {
            newIndex = carouselContentData.length - 1; // Wrap around to end
        } else if (newIndex >= carouselContentData.length) {
    ...
    // --- Funciones del Carrusel (Pensamiento y Acción) --- NEW
    function initializeContentCarousel() {
        ...
        filteredContentData.forEach((item, index) => {
            ...
            slide.className = 'content-carousel-slide flex-none w-full md:w-1/2 lg:w-1/3 p-4 box-border';
            slide.innerHTML = `
                <div class="content-item content-item-body bg-white rounded-lg shadow-md overflow-hidden transform transition-transform duration-300 hover:scale-105 h-full flex flex-col">
    ...
    function moveToContentCarouselSlide(index) {
        ...
        // Ensure newIndex is within bounds considering slidesPerView
        if (newIndex < 0) {
            newIndex = Math.max(0, filteredContentData.length - slidesPerView); // Go to last possible visible set
        } else if (newIndex > filteredContentData.length - slidesPerView) {
            newIndex = 0; // Wrap around to beginning
        }
    ...
    function updateContentCarouselPosition() {
        ...
        // Get the width of a single slide item (including padding/margin if any, handled by box-sizing)
        const firstSlide = contentCarouselTrack.querySelector('.content-carousel-slide');
        if (!firstSlide) return; // No slides yet
        const slideWidth = firstSlide.offsetWidth; // This accounts for padding as well due to box-border
    ...
    window.addEventListener('resize', function() {
        ...
        if (carouselTrack) {
            moveToCarouselSlide(currentCarouselIndex); // Recalculate position for Trayectoria
        }
        if (contentCarouselTrack) {
            updateContentCarouselPosition(); // Recalculate position for Pensamiento y Acción
            updateContentCarouselDots(); // Update dots on resize (needed for slides per view change)
        }
    });
