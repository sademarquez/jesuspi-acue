document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeCarousel() {
        ...
        if(carouselNext) carouselNext.addEventListener('click', () => { // Corrected: removed extra {
            moveToCarouselSlide(currentCarouselIndex + 1);
            resetAutoSlide();
        });
    ...
    window.addEventListener('resize', function() {
        ...
        if (carouselTrack) {
            // Recalculate position for Trayectoria
            ...
            slideElements.forEach(slide => {
                slide.style.width = `calc(${100 / slidesPerView}% - ${2 * (slidesPerView - 1) / slidesPerView}rem)`; // Adjusted margin calculation
            });
            moveToCarouselSlide(currentCarouselIndex);
        }
        if (contentCarouselTrack) {
            // Recalculate position for Pensamiento y Acción
            let slidesPerView = 1;
            if (window.innerWidth >= 1024) { // lg
                slidesPerView = 3;
            } else if (window.innerWidth >= 768) { // md
