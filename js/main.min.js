document.addEventListener('DOMContentLoaded', function () {
    ...
    function showSlide(index) {
        ...
        // const slideWidth = carouselTrack.children[0].clientWidth; // This was not always reliable

        // Desplazamiento basado en el índice y el ancho del contenedor visible del carrusel
        carouselTrack.scrollTo({
            left: currentIndex * carouselTrack.offsetWidth, // Use offsetWidth for reliable width
            behavior: 'smooth'
        });
    ...
    function startAutoSlide() {
        ...
        autoSlideInterval = setInterval(() => {
            nextSlide();
        }, 2000); // <-- VELOCIDAD DEL CARRUSEL: 2000ms (2 segundos) para rápido
                  // Ajusta este valor en milisegundos para variar la velocidad:
                  // - Menor valor (ej. 1500) = más rápido
                  // - Mayor valor (ej. 4000) = más lento
    }
    ...
    function closeMobileMenu() {
        if (mobileMenu && mobileMenu.classList.contains('active')) {
            ...
            // Esperar a que la transición termine antes de ocultar completamente con display: none
            mobileMenu.addEventListener('transitionend', function handler() {
                mobileMenu.style.display = 'none';
                mobileMenu.removeEventListener('transitionend', handler); // Limpiar el listener
            });
    ...
    if (mobileMenuButton && mobileMenu) {
        mobileMenuButton.addEventListener('click', (event) => {
            ...
            if (mobileMenu.classList.contains('active')) {
                ...
                mobileMenu.style.display = 'flex'; // Primero hazlo visible para que la transición de transform funcione
                // Usa requestAnimationFrame para asegurar que el cambio de display se aplique
                // antes de añadir la clase 'active' y disparar la transición de transform.
    ...
    // --- Cierre automático y gestión de display al redimensionar la ventana a escritorio ---
    window.addEventListener('resize', () => {
        if (window.innerWidth >= 768) { // Asumiendo 768px como el breakpoint para escritorio
            closeMobileMenu(); // Asegura que el estado del menú móvil se reinicie
            mobileMenu.style.display = ''; // Elimina el estilo 'display' inline para que CSS del media query tome el control
        } else {
            // Si estamos en móvil y el menú no está activo, asegúrate de que esté hidden
            if (!mobileMenu.classList.contains('active')) {
                mobileMenu.style.display = 'none';
            }
        }
    ...
    // Initial check for desktop view to ensure menu is displayed correctly on load
    if (window.innerWidth >= 768) {
        mobileMenu.style.display = ''; // Let CSS media query handle it
    } else {
        mobileMenu.style.display = 'none'; // Ensure it's hidden on mobile load
    }
});
