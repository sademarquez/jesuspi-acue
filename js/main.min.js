document.addEventListener('DOMContentLoaded', function () {
    ...
    // IMPORTANT: Las rutas de las imágenes deben ser relativas a la carpeta 'images/'
    ...
    if (contentGrid) {
        fetch('content.json') // Asegúrate de que esta ruta sea correcta
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status} - No se pudo cargar content.json`);
                }
            ...
            .then(data => {
                ...
                // Trigger the "Todos" filter button to ensure it's active and content is loaded
            ...
            .catch(error => {
                console.error('Error al cargar el contenido (content.json):', error);
                if (contentGrid) {
    ...
    function loadContent(dataToDisplay) {
        ...
        dataToDisplay.forEach((item, index) => {
            ...
            // Use item.image directly as it's a full URL from content.json (e.g., https://placehold.co/...)
            // The onerror will provide a fallback if the URL fails.
            card.innerHTML = `
        ...
        // Observer for Lazy Loading of content cards images (only if contentGrid exists)
        if (contentGrid.children.length > 0) { // Only observe if there are cards
    ...
    // This part is now handled in the fetch .then() block to ensure data is loaded first
    // const defaultFilterButton = document.querySelector('.filter-btn[data-category="all"]');
    // if (defaultFilterButton) {
    //     defaultFilterButton.click();
    // }
    ...
    function initializeCarousel() {
        if (!carouselTrack || carouselContentData.length === 0) {
            console.warn('Carrusel no inicializado: carouselTrack no encontrado o no hay datos.');
            if (carouselTrack) {
                // If no carousel data, display a default static image if track exists
                carouselTrack.innerHTML = `<div class="w-full h-full bg-cover bg-center flex items-center justify-center text-center" style="background-image: url('images/hero-bg.jpg');">
                                                <h3 class="text-3xl text-white text-shadow p-4 bg-deep-charcoal bg-opacity-70 rounded-lg">No hay momentos clave disponibles.</h3>
                                            </div>`;
        ...
        carouselContentData.forEach((item, index) => {
            ...
            slide.style.backgroundImage = `url('images/${item.image}')`; // Correct image path relative to images/
            slide.setAttribute('role', 'group');
            slide.setAttribute('aria-roledescription', 'slide');
            slide.setAttribute('aria-label', `${index + 1} de ${carouselContentData.length}`);
    ...
    function goToSlide(index) {
        if (index >= carouselContentData.length) {
            currentIndex = 0; // Loop to the first slide
        } else if (index < 0) {
            currentIndex = carouselContentData.length - 1; // Loop to the last slide
        } else {
            currentIndex = index;
        }
        ...
        carouselDotsContainer.querySelectorAll('button').forEach((dot, i) => {
            if (i === currentIndex) {
                ...
                dot.classList.remove('bg-gray-400'); // Ensure inactive color is removed
                dot.setAttribute('aria-selected', 'true');
            } else {
                dot.classList.remove('active-dot');
                dot.classList.add('bg-gray-400'); // Ensure inactive color is present
