document.addEventListener('DOMContentLoaded', function () {
    ...
    let currentTranslate = 0; // Para el seguimiento del arrastre
    ...
    function showSlide(index) {
        ...
        // Calcular el desplazamiento basado en el ancho del slide y el margen (si aplica)
        // Para asegurar que el slide se centre correctamente en la vista.
        const slideWidth = carouselTrack.children[0].clientWidth; // Ancho de un slide
        const slideMarginRight = parseFloat(window.getComputedStyle(carouselTrack.children[0]).marginRight); // Margen derecho
        const slideTotalWidth = slideWidth + slideMarginRight; // Ancho total incluyendo margen

        // Desplazamiento basado en el índice y el ancho total del slide
        // Esto asume que el carrusel mostrará slides completos o una cantidad específica
        // Si el carrusel se "parte" en pantallas grandes, esto puede necesitar ajuste.
        // Para la configuración actual (100% de ancho del slide), es simple:
        carouselTrack.scrollTo({
            left: currentIndex * carouselTrack.clientWidth, // Desplaza por el ancho del track
            behavior: 'smooth'
        });
    ...
    // --- Lógica de Swipe (Mantener igual) ---
    ...
    function startAutoSlide() {
        ...
        autoSlideInterval = setInterval(() => {
            nextSlide();
        }, 2000); // <-- AQUÍ PUEDES VARIAR LA VELOCIDAD
                  // El valor es en milisegundos.
                  // 1000 ms = 1 segundo.
                  // Para una velocidad "media" que permita ver el historial, 3000ms (3 segundos) es un buen punto de partida.
                  // Si quieres más rápido, disminuye el número (ej. 2000 para 2 segundos).
                  // Si quieres más lento, aumenta el número (ej. 4000 para 4 segundos).
    }
    ...
    // (Esta es la parte clave de la mejora para el carrusel)
    fetch('timeline-data.json') // Asegúrate que esta ruta sea correcta
        ...
        .catch(error => {
            ...
            // Si hay un error, el carrusel podría mostrarse vacío o con un mensaje de error.
            // Considera añadir un fallback UI aquí.
        });
    ...
    // --- Header Scroll y Menú Móvil (Mantener igual) ---
    // (Este código ya estaba funcionando bien)
    if (mobileMenuButton && mobileMenu) {
        mobileMenuButton.addEventListener('click', () => {
            mobileMenu.classList.toggle('active');
            // Alternar el icono de la hamburguesa/cerrar
    ...
    // Asume que content.json ya está disponible o se cargará más tarde.
    // Esta parte ya estaba funcional, la dejo como está.
    let contentData = []; // Variable para almacenar los datos de content.json
    ...
    function filterContent(category) {
        ...
        filterButtons.forEach(button => {
            ...
            button.classList.add('bg-muted-gold', 'text-deep-charcoal');
        });
        const activeButton = document.querySelector(`.filter-btn[data-filter="${category}"]`);
        if (activeButton) {
            activeButton.classList.remove('bg-muted-gold', 'text-deep-charcoal');
            activeButton.classList.add('bg-dark-gold', 'text-soft-cream');
        }
    }
    ...
    fetch('content.json')
        ...
        .then(data => {
            ...
            // Asigna listeners a los botones de filtro
            filterButtons.forEach(button => {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            // Cerrar menú móvil si está abierto y es un enlace de navegación
            if (mobileMenu && mobileMenu.classList.contains('active') && this.closest('#mobile-menu')) {
                mobileMenu.classList.remove('active');
                mobileMenuButton.querySelector('svg:nth-child(1)').classList.remove('hidden');
                mobileMenuButton.querySelector('svg:nth-child(2)').classList.add('hidden');
            }
        });
    });
});
