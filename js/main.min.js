document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeCarousel() {
        ...
        carouselContentData.forEach((item, index) => {
            ...
            slide.style.width = '100%'; // Default to full width on mobile
            slide.innerHTML = `
        ...
        // Adjust slide width based on screen size for Trayectoria carousel
        function updateTrayectoriaSlideWidth() {
            let slidesPerView = 1;
            ...
            const slideElements = carouselTrack.querySelectorAll('.carousel-slide');
            slideElements.forEach(slide => {
                slide.style.width = `calc(${100 / slidesPerView}% - ${1.5 * (slidesPerView - 1) / slidesPerView}rem)`; // Adjust for margin
            });
            moveToCarouselSlide(currentCarouselIndex); // Re-position after resize
        }
        window.addEventListener('resize', updateTrayectoriaSlideWidth);
        updateTrayectoriaSlideWidth(); // Initial call
    ...
    function moveToCarouselSlide(index) {
        ...
        if (newIndex < 0) {
            newIndex = totalSlides - slidesPerView; // Go to last possible visible set
        } else if (newIndex >= totalSlides - slidesPerView + 1) { // Check if we're past the last possible starting index
    ...
    function initializeContentCarousel() {
        ...
        filteredContentData.forEach((item, index) => {
            const slide = document.createElement('div');
            // Ensure proper width for carousel item on different screen sizes
            slide.className = 'content-carousel-slide flex-none p-4 box-border'; // Added p-4 for internal spacing
            slide.style.width = '100%'; // Default for mobile, adjusted by JS for larger screens
            slide.innerHTML = `
        ...
        // Adjust slide width based on screen size for Content carousel
        function updateContentSlideWidth() {
            let slidesPerView = 1;
            if (window.innerWidth >= 1024) { // lg
                slidesPerView = 3;
            } else if (window.innerWidth >= 768) { // md
            ...
            const slideElements = contentCarouselTrack.querySelectorAll('.content-carousel-slide');
            slideElements.forEach(slide => {
                // Calculate width considering the 1rem margin on each side (2rem total per slide)
                slide.style.width = `calc(${100 / slidesPerView}% - ${2 * (slidesPerView - 1) / slidesPerView}rem)`;
            });
            moveToContentCarouselSlide(currentContentCarouselIndex); // Re-position after resize
        }
        window.addEventListener('resize', updateContentSlideWidth);
        updateContentSlideWidth(); // Initial call
    ...
    function updateContentCarouselPosition() {
        ...
        // Calculate translation, ensuring it doesn't scroll past the end
    ...
    window.addEventListener('resize', function() {
        ...
        if (carouselTrack) {
            // Recalculate position for Trayectoria, calling updateTrayectoriaSlideWidth
            const slideElements = carouselTrack.querySelectorAll('.carousel-slide');
            let slidesPerView = 1;
            if (window.innerWidth >= 1024) { // lg
                slidesPerView = 3;
            ...
            slideElements.forEach(slide => {
                slide.style.width = `calc(${100 / slidesPerView}% - ${1.5 * (slidesPerView - 1) / slidesPerView}rem)`;
            });
            moveToCarouselSlide(currentCarouselIndex);
        }
        if (contentCarouselTrack) {
            // Recalculate position for Pensamiento y Acción, calling updateContentSlideWidth
            let slidesPerView = 1;
            if (window.innerWidth >= 1024) { // lg
                slidesPerView = 3;
            } else if (window.innerWidth >= 768) { // md
