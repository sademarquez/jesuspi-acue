document.addEventListener('DOMContentLoaded', function () {
    ...
    function showSlide(index) {
        ...
        // Desplazamiento basado en el índice y el ancho del slide visible
        // `carouselTrack.clientWidth` es el ancho del contenedor visible
        carouselTrack.scrollTo({
            left: currentIndex * carouselTrack.clientWidth, 
            behavior: 'smooth'
        });
    ...
    if (mobileMenuButton && mobileMenu) {
        mobileMenuButton.addEventListener('click', () => {
            ...
            // --- INICIO DE LA CORRECCIÓN DEL MENÚ BURGER ---
            // Asegúrate que el SVG de la hamburguesa se oculte y el de cerrar se muestre.
            // Si el problema es de visibilidad inicial, el CSS también necesita un ajuste.
            const hamburgerIcon = mobileMenuButton.querySelector('.hamburger-icon');
            const closeIcon = mobileMenuButton.querySelector('.close-icon');

            if (mobileMenu.classList.contains('active')) {
                // Menú abierto: Oculta hamburguesa, muestra cerrar
                if (hamburgerIcon) hamburgerIcon.classList.add('hidden');
                if (closeIcon) closeIcon.classList.remove('hidden');
            } else {
                // Menú cerrado: Muestra hamburguesa, oculta cerrar
                if (hamburgerIcon) hamburgerIcon.classList.remove('hidden');
                if (closeIcon) closeIcon.classList.add('hidden');
            }
            // --- FIN DE LA CORRECCIÓN DEL MENÚ BURGER ---
        });
    }
    ...
    // Esta sección está completa y funciona con content.json
    let contentData = []; 
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            if (mobileMenu && mobileMenu.classList.contains('active') && this.closest('#mobile-menu')) {
                ...
                // Asegúrate de restaurar los iconos al cerrar el menú desde un enlace
                const hamburgerIcon = mobileMenuButton.querySelector('.hamburger-icon');
                const closeIcon = mobileMenuButton.querySelector('.close-icon');
                if (hamburgerIcon) hamburgerIcon.classList.remove('hidden');
                if (closeIcon) closeIcon.classList.add('hidden');
